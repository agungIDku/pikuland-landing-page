import type { FooterContent } from "@/types/footerContent";
import Image from "next/image";
import Link from "next/link";

const SOCIAL_ORDER = ["instagram", "facebook", "youtube"] as const;

type FooterProps = {
  content: FooterContent;
};

export default function Footer({ content: c }: FooterProps) {
  const logoSrc = c.logoUrl?.trim();

  return (
    <footer className="bg-[#121926] text-white pt-16 md:pt-24 pb-8 md:pb-12 px-6">
      <div className="max-w-6xl mx-auto">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-0 sm:gap-12 md:gap-8 pb-10 md:pb-16 border-b border-white/10">
          {/* Brand column — full width on mobile */}
          <div className="col-span-2 lg:col-span-1 pb-8 sm:pb-0 border-b border-white/10 sm:border-none">
            {logoSrc ? (
              <Image
                src={logoSrc}
                alt="Pikuland Logo"
                width={100}
                height={60}
                className="h-10 md:h-12 w-auto"
              />
            ) : null}
            <p className="mt-4 text-sm text-gray-400 leading-relaxed max-w-xs font-medium">
              {c.tagline}
            </p>
            {/* Social icons — only platforms present in API */}
            <div className="mt-6 flex flex-wrap gap-3">
              {SOCIAL_ORDER.map((social) => {
                const href = c.social[social]?.trim();
                if (!href) return null;
                return (
                  <a
                    key={social}
                    href={href}
                    className="w-10 h-10 rounded-full bg-slate-800/80 hover:bg-slate-700 flex items-center justify-center text-gray-400 hover:text-white transition-colors"
                    aria-label={social}
                    {...(href.startsWith("http")
                      ? { target: "_blank", rel: "noopener noreferrer" }
                      : {})}
                  >
                    <SocialIcon name={social} />
                  </a>
                );
              })}
            </div>
          </div>

          {/* Jelajahi */}
          <div className="py-8 sm:py-0 border-b border-white/10 sm:border-none">
            <h3 className="font-extrabold text-base mb-5">{c.exploreTitle}</h3>
            <ul className="space-y-4">
              {c.exploreLinks.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="text-sm text-gray-400 hover:text-white transition-colors"
                  >
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Info Penting */}
          <div className="py-8 sm:py-0 border-b border-white/10 sm:border-none">
            <h3 className="font-extrabold text-base mb-5">
              {c.importantTitle}
            </h3>
            <ul className="space-y-4">
              {c.importantLinks.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="text-sm text-gray-400 hover:text-white transition-colors"
                  >
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Hubungi Kami */}
          <div className="col-span-1 pt-8 sm:pt-0">
            <h3 className="font-extrabold text-base mb-5">{c.contactTitle}</h3>
            <ul className="space-y-4 text-sm text-gray-400">
              <li>{c.addressLine}</li>
              <li>{c.email}</li>
              <li>{c.phone}</li>
            </ul>
            <a
              href={c.whatsappHref}
              target="_blank"
              rel="noopener noreferrer"
              className="mt-6 flex items-center justify-center w-full sm:w-auto sm:inline-flex bg-[#00A1E4] text-white font-bold text-sm px-8 py-3.5 rounded-full hover:bg-[#008AC4] transition-colors"
            >
              {c.whatsappLabel}
            </a>
          </div>
        </div>

        {/* Copyright */}
        {c.copyright ? (
          <p className="mt-8 text-center text-xs text-gray-500 font-medium">
            {c.copyright}
          </p>
        ) : null}
      </div>
    </footer>
  );
}

function SocialIcon({ name }: { name: string }) {
  switch (name) {
    case "instagram":
      return (
        <svg
          className="w-4 h-4 md:w-5 md:h-5"
          fill="currentColor"
          viewBox="0 0 24 24"
        >
          <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
        </svg>
      );
    case "facebook":
      return (
        <svg
          className="w-4 h-4 md:w-5 md:h-5"
          fill="currentColor"
          viewBox="0 0 24 24"
        >
          <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
        </svg>
      );
    case "youtube":
      return (
        <svg
          className="w-4 h-4 md:w-5 md:h-5"
          fill="currentColor"
          viewBox="0 0 24 24"
        >
          <path d="M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
        </svg>
      );
    default:
      return null;
  }
}
